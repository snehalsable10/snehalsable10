# 👋 Hello, I'm Snehal Sable  

I am a final-year **Computer Engineering student at Sinhgad Institute of Technology and Science (SITS), Pune**.  
I love building full-stack applications, exploring AI/ML, and solving real-world problems through clean and efficient code.  
💡 *“Consistency makes you unstoppable.”*

I’m passionate about **Full-Stack Development, Java, NLP-based AI applications, and Databases**.  
💬 Feel free to ask me anything related to **MERN, Java, SQL, APIs, Cloud, or DSA!**

---

## 💼 Work Experience

| Company/Organization | Role | Duration |
|---------------------|-------|----------|
| **Scaleful Technologies, Pune** | Software Developer Intern (MERN) | Dec 2024 – Feb 2025 |

---

## 🚀 Projects  

### 🍔 **Feastify – Food Order Platform (MERN)**  
A full-stack food ordering system with authentication, admin dashboard, and live order tracking.

### 🤖 **AI Resume Analyzer (Python + NLP)**  
Extracts skills, analyzes resume content, generates ATS score, and recommends job roles.

### 📈 **Trading Platform Dashboard (MERN)**  
A stock trading mock dashboard with real-time charts, user portfolio, and JWT authentication.

---

## 🛠 Tech Stack  

### **Languages:**  
- Java, JavaScript, Python  

### **Frontend:**  
- React.js, HTML/CSS, Tailwind  

### **Backend:**  
- Node.js, Express.js  

### **Database:**  
- MongoDB, MySQL  

### **AI/ML:**  
- NLP, Scikit-Learn  

### **Tools:**  
- Git, GitHub, Postman, Render, Vercel, VS Code  

---

## 🏆 Achievements  

- Led a **10+ member team** during the *Fast & Furious* event in TechTonic.  
- Participant in **Flipkart Grid 6.0 – Software Development Track**.  
- Built multiple real-world projects in MERN & AI/ML.  

---

## ✍️ Developer Quote  
> *“First, solve the problem. Then, write the code.”* — John Johnson

---

## 📫 Contact Me  

- ✉️ **Email:** snehalsable105@gmail.com  
- 🌐 **Portfolio:** https://portfolio-website-utwm.vercel.app/  
- 🔗 **LinkedIn:** *(Share your link if you want it added)*  
- 💻 **GitHub:** *(Share your username if you want it added)*  

---

⭐ *Thanks for visiting my profile! Feel free to explore my projects and connect with me.*  
